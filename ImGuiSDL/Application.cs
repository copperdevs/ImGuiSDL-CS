...
internal unsafe class Application : IDisposable
{
    /// <summary>
    /// The SDL Window
    /// </summary>
    ...
    {
        // launch SDL
        if (!SDLAPI.Init(InitFlags.Video))
            throw new Exception($"{nameof(SDLAPI.Init)} Failed: {SDLAPI.GetError()}");
        ...
        Window = SDLAPI.CreateWindow(name, width, height, windowFlags);
        if (Window == null)
            throw new Exception($"{nameof(SDLAPI.CreateWindow)} Failed: {SDLAPI.GetError()}");

        // claim SDL GPU Window
        if (!SDLAPI.ClaimWindowForGpuDevice(Device, Window))
    ...
    public void Dispose()
    {
        ...
        Running = false;
        Renderer.Dispose();
        SDLAPI.ReleaseWindowFromGpuDevice(Device, Window);
    ...
    private void PollEvents()
    {
        ...
        if (ImGui.GetIO().WantTextInput && !SDLAPI.TextInputActive(Window))
            SDLAPI.StartTextInput(Window);
        else if (!ImGui.GetIO().WantTextInput && SDLAPI.TextInputActive(Window))
            SDLAPI.StopTextInput(Window);

        // poll SDL events
        while (SDLAPI.PollEvent(out var ev))
        {
